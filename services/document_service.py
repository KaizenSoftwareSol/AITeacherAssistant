...
class DocumentService:
    ...
    async def process_website_upload(
        db, teacher: Teacher, url: str, title: str, description: Optional[str] = None
    ) -> DocumentRead:
        """
        ...
        try:
            ...
            json_content = json.dumps(
                website_content.dict(), indent=2, ensure_ascii=False
            )
            await DocumentService.upload_file_to_supabase(
                json_content.encode("utf-8"), json_path, "application/json"
            )
                ...
                document_metadata=json.dumps(
                    {
                        ...
                        "word_count": (
                            website_content.metadata.get("word_count", 0)
                            if website_content.metadata
                            else 0
                        ),
                    }
                ),
